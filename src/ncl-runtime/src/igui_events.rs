//! Event mailbox: GUI thread → language thread(s). **Platform-neutral.**
//!
//! Extracted verbatim from `igui::channels` (which had zero `windows::`
//! coupling — only a `#![cfg(windows)]` gate) so the macOS backend
//! (`igui_mac`) shares the exact same mailbox and `IGuiEvent` enum.
//! `igui::channels` now re-exports this module. See PORTING_DESIGN.md §4.5.
//!
